...
const initialResumes: Resume[] = [
    {
        ...
        category: 'tech',
        fileName: 'SWE_Resume_2026.pdf',
        fileFormat: 'PDF',
        fileSize: '245 KB',
        uploadDate: '2026-02-15',
        tags: ['React', 'TypeScript', 'Full Stack'],
        description: 'Comprehensive resume for full-stack software engineer positions.',
        isPrimary: true,
    },
    {
        id: '2',
        ...
        category: 'tech',
        fileName: 'FullStack_Resume_2026.docx',
        fileFormat: 'DOCX',
        fileSize: '189 KB',
    ...
    },
    {
        ...
        category: 'general',
        fileName: 'General_Resume_2026.pdf',
        fileFormat: 'PDF',
        fileSize: '156 KB',
        uploadDate: '2025-12-10',
        tags: ['Leadership', 'Communication', 'Problem Solving'],
        description: 'General purpose resume for broader applications.',
        isPrimary: false,
    },
];
...
function ResumesPage() {
    ...
    const filteredResumes = useMemo(() => {
        ...
        let filtered = resumes.filter((resume) => {
            const matchesSearch =
                ...
                resume.description?.toLowerCase().includes(normalizedSearch);
    ...
    // Get unique titles, categories and file formats
    const uniqueTitles = useMemo(() => {
        return Array.from(new Set(
            resumes.map((resume) => resume.title).filter(Boolean)
        )).sort();
    }, [resumes]);

    const uniqueCategories = useMemo(() => {
        return Array.from(new Set(
            resumes.map(resume => resume.category).filter(Boolean)
        )).sort();
    }, [resumes]);

    const uniqueFormats = useMemo(() => {
        return Array.from(new Set(
            resumes.map(resume => resume.fileFormat).filter(Boolean)
        )).sort();
    }, [resumes]);
    ...
    const getFilterButtonText = () => {
        ...
        if (filterField === 'category') {
            return categoryLabels[filterValue as ResumeCategory];
        }
        return filterValue;
    };
    ...
    const handleDownloadResume = (resume: Resume) => {
        ...
        const fileContent = `Resume: ${resume.title}\nCategory: ${categoryLabels[resume.category]}\nUploaded: ${resume.uploadDate}\n`;
                    ...
                    <HStack justify="space-between" align="flex-start" flexWrap="wrap" gap={2}>
                        <Box>
                            ...
                            <Text color="gray.600" fontSize={{ base: 'xs', md: 'sm' }}>
                                Manage and organize your resume versions for different job applications.
                            </Text>
                        </Box>
                    ...
                    <VStack spacing={6} py={16} align="center" justify="center" bg="gray.50" borderRadius="lg" minH="300px">
                        ...
                        <VStack spacing={2} align="center">
                            ...
                            <Text color="gray.600" maxW="sm" textAlign="center">
                                Upload your first resume to get started with version management and skill tracking.
                            </Text>
                        </VStack>
                        <Button
                            leftIcon={<AddIcon />}
                                                    ...
                                                    <AccordionPanel p={0} maxH="200px" overflowY="auto" bg="gray.50">
                                                        {uniqueTitles.map((title, index) => (
                                                            <Box key={title}>
                                                                ...
                                                                <MenuItem
                                                                    onClick={() => handleFilterChange('title', title)}
                                                                    bg={filterField === 'title' && filterValue === title ? 'brand.50' : 'transparent'}
                                                                    fontWeight={filterField === 'title' && filterValue === title ? 'semibold' : 'normal'}
                                                                    fontSize="14px"
                                                                    _hover={{ bg: filterField === 'title' && filterValue === title ? 'brand.100' : 'gray.100' }}
                                                                >
                                                                    {title}
                                                                </MenuItem>
                                                            </Box>
                                                    ...
                                                    {uniqueCategories.map((category, index) => (
                                                        <Box key={category}>
                                                            ...
                                                            <MenuItem
                                                                ...
                                                                bg={filterField === 'category' && filterValue === category ? 'brand.50' : 'transparent'}
                                                                fontWeight={filterField === 'category' && filterValue === category ? 'semibold' : 'normal'}
                                                                fontSize="14px"
                                                                _hover={{ bg: filterField === 'category' && filterValue === category ? 'brand.100' : 'gray.100' }}
                                                            >
                                                                <Badge colorScheme={categoryColors[category]} mr={2}>
                                                    ...
                                                    <AccordionPanel p={0} maxH="200px" overflowY="auto" bg="gray.50">
                                                        {uniqueFormats.map((format, index) => (
                                                            <Box key={format}>
                                                                {index > 0 && <MenuDivider my={0} />}
                                                                <MenuItem
                                                                    onClick={() => handleFilterChange('fileFormat', format)}
                                                                    bg={filterField === 'fileFormat' && filterValue === format ? 'brand.50' : 'transparent'}
                                                                    fontWeight={filterField === 'fileFormat' && filterValue === format ? 'semibold' : 'normal'}
                                                                    fontSize="14px"
                                                                    _hover={{ bg: filterField === 'fileFormat' && filterValue === format ? 'brand.100' : 'gray.100' }}
                                                                >
                                                                    {format}
                                                                </MenuItem>
                                                            </Box>
                            ...
                            <SortableDataTable
                                ...
                                renderRow={(resume) => (
                                    <Tr key={resume.id}>
                                        ...
                                        <Td fontSize="14px">
                                            <Badge colorScheme={categoryColors[resume.category]}>
                                                {categoryLabels[resume.category]}
                                            </Badge>
                                        </Td>
                                        ...
                                        <Td fontSize="14px" position="relative" pe="0px">
                                            ...
                                            <Menu>
                                                ...
                                                <MenuList>
                                                    ...
                                                    <MenuDivider my={1} />
                                                    <MenuItem onClick={() => handleDownloadResume(resume)} fontSize="14px" icon={<DownloadIcon />}>
                                                        Download
                                                    </MenuItem>
                                                </MenuList>
                                            </Menu>
                            ...
                            {selectedResume && (
                                <VStack spacing={4} align="stretch">
                                    ...
                                    <FormControl>
                                        ...
                                        <Input
                                            value={categoryLabels[selectedResume.category]}
                                            isReadOnly
                                        />
                                    </FormControl>
                                    <FormControl>
                                    ...
                                    <FormControl>
                                        ...
                                        <Input
                                            value={selectedResume.fileFormat}
                                            isReadOnly
                                        />
                                    </FormControl>
                                    <FormControl>
                                    ...
                                    <FormControl>
                                        ...
                                        <Input
                                            value={selectedResume.uploadDate}
                                            isReadOnly
                                        />
                                    </FormControl>
                                    {selectedResume.description && (
                                        <FormControl>
                                            ...
                                            <Box p={3} bg="gray.50" borderRadius="md">
                                                <Text fontSize="14px">{selectedResume.description}</Text>
                                            </Box>
                                        </FormControl>
                                    )}
                                </VStack>
